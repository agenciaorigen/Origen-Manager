...
export interface CalItem { id: string; date: string; time: string; label: string; sub?: string; tone: Tone; done?: boolean; onClick(): void }
...
function Item({ i, compact }: { i: CalItem; compact?: boolean }) {
  return (
    <button onClick={i.onClick} className={cx('flex w-full items-center gap-1.5 rounded-lg text-left hover:bg-bg', compact ? 'px-1 py-0.5 text-[11px]' : 'px-2 py-1.5 text-sm', i.done && 'opacity-50 line-through')}>
      <span className={cx('size-1.5 shrink-0 rounded-full', DOT[i.tone])} />
      {i.time && <span className="font-mono text-[10px] text-mute">{i.time}</span>}
      <span className="truncate">{i.label}</span>
      {!compact && i.sub && <span className="ml-auto truncate text-xs text-mute">{i.sub}</span>}
    </button>
  )
}
            ...
            {days.map(d => {
              ...
              return (
                <div key={d} className={cx('min-h-16 bg-card p-1 sm:min-h-24', !inM && 'bg-bg/60 text-mute/60')}>
                  ...
                  <div className="flex flex-wrap gap-0.5 px-0.5 sm:hidden">{its.slice(0, 4).map(i => <span key={i.id} className={cx('size-1.5 rounded-full', DOT[i.tone])} />)}</div>
                </div>
              )
            })}
          </div>
