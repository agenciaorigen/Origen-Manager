...
export function buildReminders(db: DB, now: ISODate = today()): Reminder[] {
  ...
  if (late) out.push({ id: 'late', level: 'alert', text: `Tenés ${late} ${late === 1 ? 'tarea atrasada' : 'tareas atrasadas'}.`, link: '/' })
  ...
  for (const t of db.tasks.filter(t => t.status === 'pendiente' && t.date === now))
    out.push({ id: `t-${t.id}`, level: 'info', text: `Hoy: ${t.title} — ${name(t.client_id)}.`, link: '/' })
